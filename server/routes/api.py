...
from server.database.models import User, Project, Organisation
import json
...
def api_signup():
    ...
    if not data or not all(k in data for k in ('nom', 'prenom', 'email', 'password')):
...
def api_create_project():
    ...
    if not data or not all(k in data for k in ('numero', 'nom')):
        return jsonify({'success': False, 'message': 'Missing required fields'}), 400
    ...
    org_id = User.get_user_org_id(user_id)
    ...
    try:
        project = Project(
            ...
            org_id=org_id  # Set the organization ID here
        )
